// ⚠️ GANTI DENGAN URL DEPLOYMENT GOOGLE APPS SCRIPT BOS YANG TERBARU
...
// 👑 IDENTITAS FIX APLIKASI (JANGAN DIUBAH)
const LOKASI_MPP = "WALMAS";
...
window.onload = () => {
    // Inject Tahun Dinamis untuk Copyright
    document.getElementById('dynamicYear').textContent = new Date().getFullYear();

    const currentDate = new Date();
    document.getElementById('operasionalDate').valueAsDate = currentDate;
    
    let mondayOffset = new Date();
    mondayOffset.setDate(currentDate.getDate() - (currentDate.getDay() === 0 ? 6 : currentDate.getDay() - 1));
    document.getElementById('datetimeStart').valueAsDate = mondayOffset;
    document.getElementById('datetimeEnd').valueAsDate = currentDate;
...
function dispatchDataToCloud() {
    ...
    .then(() => {
        ...
        setTimeout(() => { 
            ...
            btn.innerHTML = `<i class="fa-solid fa-cloud-arrow-up"></i> TRANSMISI KE SERVER PUSAT`; 
            btn.disabled = false;
        }, 1500);
    }).catch(() => {
...
// 👑 FUNGSI ENTERPRISE: Pengubah Tanggal Internasional ke Format Resmi Indonesia
...
function executeCloudQuery() {
    ...
    // 👑 LOGIKA ANTI-CACHE: Memastikan sistem selalu menarik data Excel terbaru
...
function compileAnalyticReporting(rawCollection) {
        ...
        if(row.tanggal) {
            ...
            if(rowTime >= startTime && rowTime <= endTime) {
                opdRegistry.forEach(o => {
                    ...
                    if(cellValue > 0) {
                        if(row.lokasi === "MPP_WALMAS") internalMatrix[o.id].walmas += cellValue;
                        else internalMatrix[o.id].belopa += cellValue;
    ...
    // 👑 Menerapkan Format Tanggal Indonesia
    ...
    processedSequence.forEach((item, index) => {
        dataInjectorHtml += `
            <div class="table-row-premium">
                ...
                <div class="col-belopa">
                    ${item.belopa > 0 ? `<span class="badge-belopa">${item.belopa.toLocaleString('id-ID')}</span>` : '-'}
                </div>
                <div class="col-walmas">
                    ${item.walmas > 0 ? `<span class="badge-walmas">${item.walmas.toLocaleString('id-ID')}</span>` : '-'}
                </div>
